...
from app.domain.stocks.instrument_type import required_histories, inapplicable_reason
...
def dependency_quality(fundamentals: dict, institutional: dict, rs: dict, *, today: date | None = None) -> dict[str, dict]:
    ...
    instrument_type = fundamentals.get("instrument_type", "unknown")
    needed = required_histories(instrument_type)
    histories_complete = all(
        ...
        for key, current, previous in histories if key in needed
    )
    ...
    if report_state.get("complete") is False and not (
        report_state.get("reason_code") == "provider_rate_limited" and histories_complete
    ):
    ...
    return {
        ...
        "histories": {"status": "fresh" if histories_complete else "missing", "as_of": fundamentals.get("as_of"),
                      "label": "Fundamentalkriterium für diesen Wertpapiertyp nicht anwendbar" if inapplicable_reason(instrument_type)
                      else "EPS-/Umsatz-Jahreshistorien" if instrument_type == "foreign_private_issuer"
                      else "EPS-/Umsatz-Historien (3 Quartale und 3 Jahre)"},
    }
