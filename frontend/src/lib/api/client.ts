...
export const api = {
  ...
  reportWork: () => getJson<{ due_count: number; oldest_due_at: string | null; next_due_at: string | null; groups: { group: string; status: string; reason_code: string; count: number; due_count: number; next_due_at: string | null }[]; active: { ticker: string; group: string; lease_until: string | null }[]; provider_usage: Record<string, number>; sec_bulk_cache: { available?: boolean; fetched_at?: string; bytes?: number; error?: string } }>("/jobs/report-work"),
